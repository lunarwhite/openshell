...
| `server.databaseStorageClass` | `StorageClass` for the gateway database PVC under `workload.kind=statefulset`. Empty (default) omits `storageClassName` and uses the cluster's default `StorageClass`. |
