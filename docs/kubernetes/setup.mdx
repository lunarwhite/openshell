...
| `server.databaseStorageSize` | Storage request for the gateway database PVC under `workload.kind=statefulset`. Defaults to `1Gi`. Requires an explicit unit, such as `20Gi`. |
| `server.databaseStorageClass` | `StorageClass` for the gateway database PVC under `workload.kind=statefulset`. Empty (default) omits `storageClassName` and uses the cluster's default `StorageClass`. Use a block-backed class, not NFS. |
...
<Warning>
`server.databaseStorageSize` and `server.databaseStorageClass` feed the gateway StatefulSet's immutable `volumeClaimTemplates`. Set them at install time. Changing them later fails the upgrade and does not move existing data; refer to the [Helm chart README](https://github.com/NVIDIA/OpenShell/blob/main/deploy/helm/openshell/README.md#database-pvc-storage) for migration steps.
</Warning>
