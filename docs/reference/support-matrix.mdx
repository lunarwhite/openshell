...
| LXD | Supported for LXD-backed sandboxes on Ubuntu. Opt-in only; never auto-detected. | Requires LXD and `lxd`-group membership, which is host-root-equivalent. |
...
| LXD | 6.9 (snap) | Required for LXD-backed gateways. Verified on Ubuntu 26.04 LTS with a `dir`-backed storage pool; other Ubuntu releases, LXD versions, and storage backends are untested. |
