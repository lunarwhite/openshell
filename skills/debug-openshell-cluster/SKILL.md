...
| `helm upgrade` fails on a forbidden update to the gateway StatefulSet after changing `server.databaseStorageSize` or `server.databaseStorageClass` | `volumeClaimTemplates` is immutable on an existing StatefulSet | `kubectl -n openshell delete statefulset openshell --cascade=orphan`, then re-upgrade; the orphaned PVC keeps its original size and class, so grow it with in-place PVC expansion or copy the data to a new claim |
