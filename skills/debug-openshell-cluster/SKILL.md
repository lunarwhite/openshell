...
| Kubernetes gateway pod pending | PVC unbound, taint, selector, or insufficient resources | `kubectl -n openshell describe pod <pod>`; if the gateway PVC is unbound because the cluster has no default `StorageClass`, set `server.databaseStorageClass` to a valid one, and size the claim with `server.databaseStorageSize` |
| `helm upgrade` fails on a forbidden update to the gateway StatefulSet | `server.databaseStorageSize` or `server.databaseStorageClass` changed on an existing release; `volumeClaimTemplates` is immutable | `kubectl -n openshell delete statefulset openshell --cascade=orphan`, then re-upgrade; the orphaned PVC keeps its original size and class, so grow it with in-place PVC expansion or copy the data to a new claim |
